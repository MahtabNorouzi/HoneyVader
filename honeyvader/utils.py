...
def get_ULE_expr_args(expr, parents=None):
    arg_list = []
    if parents is None:
        parents = []
    if is_app_of(expr, Z3_OP_ULEQ):
        arg_list.append(expr.children())
    else:
        if is_app(expr):
            children = expr.children()
            parents.append(expr)
            for child in children:
                if is_app_of(child, Z3_OP_ULEQ):
                    if any([is_app_of(par, Z3_OP_NOT) for par in parents]):
                        arg_list.append(child.children())
                        arg_list[0].reverse()
                        break
                    else:
                        arg_list.append(child.children())
                    ...
                    arg_list.extend(get_ULE_expr_args(child, parents))
    return arg_list

# def get_ULE_expr_args(expr):
#     arg_list = []
#     expr_list = [expr]
#     while expr_list:
#         expr = expr_list.pop()
...
#                 arg_list.append(expr.children())
#             elif expr.children():
#                 expr_list = expr.children() + expr_list
#     return arg_list

# check if two expressions depend on each other


def expr_depends_on(expr1, expr2):
    children_q = [expr1]
    while children_q:
        child = children_q.pop()
        if is_expr(child):
            if child.sort() == expr2.sort() and is_true(simplify(child == expr2)):
                return True
            elif child.num_args() > 0:
                children_q = child.children() + children_q
    return False
    ...
    for storage_addr in global_states:
        ...
        if is_expr(expr):
            ...
            for var in list_vars:
                ...
                # check if a var is global
                if var_name.startswith("Ia_store_"):
                    position = int(var_name.split(
                        '_')[len(var_name.split('_'))-1])
...
def get_contract_info(contract_addr):
    ...
    try:
        ...
        try:
            os.system("wget -O %s http://etherscan.io/txs?a=%s" %
                      (file_name1, contract_addr))
            re_txs_value = r"<span>A total of (.+?) transactions found for address</span>"
            txs = run_re_file(re_txs_value, file_name1)

            # get balance
            re_str_value = r"<td>ETH Balance:\n<\/td>\n<td>\n(.+?)\n<\/td>"
            os.system("wget -O %s https://etherscan.io/address/%s" %
...
def get_contract_stats(list_of_contracts):
    with open("concurr.csv", "w") as stats_file:
        ...
        fp.writerow(["Contract address", "No. of paths",
                    "No. of concurrency pairs", "Balance", "No. of TXs", "Note"])
...
def remove_concat(expression):
    ...
    while len(match) > 0:
        expression = re.sub('Concat\(.*?, (.+?\))\)',
                            match[0], str(expression), 1)
        match = re.compile('Concat\(.*?, (.+?\))\)').findall(str(expression))
    return expression
...
def bv_min(bv_list):
    if len(bv_list) > 1:
        firstElement = ZeroExt((256 - bv_list[0].size()), bv_list[0])
        secondElement = ZeroExt((256 - bv_list[1].size()), bv_list[1])
        minimum = If(firstElement < secondElement, firstElement, secondElement)
        for bv in range(2, len(bv_list)):
            extended_bv = ZeroExt((256 - bv_list[bv].size()), bv_list[bv])
            minimum = If(minimum < extended_bv, minimum, extended_bv)
        return minimum
    elif len(bv_list) == 1:
        return ZeroExt((256 - bv_list[0].size()), bv_list[0])
    else:
        return None
...
def bv_max(bv_list):
    ...
    # pretty_printer.pprint(bv_list)
    if len(bv_list) > 1:
        firstElement = ZeroExt((256 - bv_list[0].size()), bv_list[0])
        secondElement = ZeroExt((256 - bv_list[1].size()), bv_list[1])
        maximum = If(firstElement > secondElement, firstElement, secondElement)
        for bv in range(2, len(bv_list)):
            extended_bv = ZeroExt((256 - bv_list[bv].size()), bv_list[bv])
            maximum = If(maximum > extended_bv, maximum, extended_bv)
        return maximum
    elif len(bv_list) == 1:
        return ZeroExt((256 - bv_list[0].size()), bv_list[0])
    else:
