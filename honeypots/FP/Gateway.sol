...
contract Gateway is Ownable {
    ...
    function getBuyerAddressPayment(address _sellerAddress, uint256 _orderId)
        public
        constant
        returns (address)
    {
    ...
    function getSellerAddressPayment(address _sellerAddress, uint256 _orderId)
        public
        constant
        returns (address)
    {
    ...
    function getValuePayment(address _sellerAddress, uint256 _orderId)
        public
        constant
        returns (uint256)
    {
    ...
    function getCurrencyPayment(address _sellerAddress, uint256 _orderId)
        public
        constant
        returns (address)
    {
    ...
    function setFeeAccount1(address _feeAccount1) public onlyOwner {
        feeAccount1 = _feeAccount1;
    }

    function setFeeAccount2(address _feeAccount2) public onlyOwner {
        feeAccount2 = _feeAccount2;
    }

    function setFeeAccountToken(address _feeAccountToken) public onlyOwner {
    ...
    function payToken(
        ...
        token.transferFrom(
            msg.sender,
            feeAccountToken,
            _value.mul(feePercent).div(100000000)
        );
        token.transferFrom(
            msg.sender,
            _sellerAddress,
            _value.sub(_value.mul(feePercent).div(100000000))
        );
        payment[_sellerAddress][_orderId] = BuyInfo(
            msg.sender,
            _sellerAddress,
            _value,
            _tokenAddress
    ...
    function payEth(
        ...
        require(_sellerAddress != address(0));
        require(_value > 0);
        uint256 fee = _value.mul(feePercent).div(100000000);
    ...
    function transferFee() public onlyOwner {
        uint256 valfee1 = balanceFee.div(2);
    ...
    function balanceOfToken(address _tokenAddress, address _Address)
        public
        constant
        returns (uint256)
    {
    ...
    function balanceOfEthFee() public constant returns (uint256) {
        return balanceFee;
    }
    ...
    function() external payable {
        ...
        uint256 value = msg.value.div(10000000000).mul(10000000000);
        uint256 orderId = msg.value.sub(value);
