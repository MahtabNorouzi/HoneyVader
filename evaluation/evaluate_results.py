...
global map_key_encodings
global racing_time_contract 
...
global list_of_map_key_encodings
global list_of_racing_contracts
...
list_of_map_key_encodings          = set()
list_of_racing_time_contracts      = set()
...
def evaluate_contract(contract):
    ...
    global list_of_straw_man_contracts
    global list_of_map_key_encodings
    global list_of_racing_time_contracts
    ...
    if contract["map_key_encoding"] != False:
        map_key_encoding = True
    if contract["racing_time"] != False:
        racing_time = True
...
for file in os.listdir(os.path.join("Ethereum","results")):
    # print("file", file)
    if file.endswith(".json"):
        total += 1
        try:
            data = json.load(open(os.path.join(os.path.join("Ethereum","results"), file)))
            ...
            if map_key_encoding:
                map_key_encodings += 1
                list_of_map_key_encodings.add(address)
            if racing_time:
                racing_time_contracts += 1
                list_of_racing_time_contracts.add(address)

            if balance_disorder or hidden_transfer or inheritance_disorder or uninitialised_struct or type_deduction_overflow or skip_empty_string_literal or hidden_state_update or straw_man_contract or map_key_encoding or racing_time:
                vulnearable += 1
...
print("Number of unique map key encodings: "+str(map_key_encodings)+" ("+str(float(map_key_encodings)/float(total)*100)+"%)")
print("Number of unique racing time contracts: "+str(racing_time_contracts)+" ("+str(float(racing_time_contracts)/float(total)*100)+"%)")
...
# list_of_bytecode_map_key_encodings = set()
# list_of_bytecode_racing_time_contracts = set()

# # cursor = collection.find()
# for file in os.listdir(os.path.join("Ethereum","ethereum_contracts")):
#     with open(file, mode='rb') as f:
#         print("file", file)
#         print(f.read())
#     contract_address = file.split('.')[0]

#     if contract_address in addresses:
#         with open(file, mode='rb') as f:
#             list_of_bytecode_txhashes.add(sha256(bytearray.fromhex(f)).hexdigest())

#     if contract_address in list_of_money_flows:
#         with open(file) as f:
#             list_of_bytecode_money_flows.add(sha256(bytearray.fromhex(f)).hexdigest())

#     if contract_address in list_of_balance_disorders:
#         with open(file) as f:
#             list_of_bytecode_balance_disorders.add(sha256(bytearray.fromhex(f)).hexdigest())

#     if contract_address in list_of_hidden_transfers:
#         with open(file) as f:
#             list_of_bytecode_hidden_transfers.add(sha256(bytearray.fromhex(f)).hexdigest())

#     if contract_address in list_of_inheritance_disorders:
#         with open(file) as f:
#             list_of_bytecode_inheritance_disorders.add(sha256(bytearray.fromhex(f)).hexdigest())

#     if contract_address in list_of_uninitialised_structs:
#         with open(file) as f:
#             list_of_bytecode_uninitialised_structs.add(sha256(bytearray.fromhex(f)).hexdigest())

#     if contract_address in list_of_type_deduction_overflows:
#         with open(file) as f:
#             list_of_bytecode_type_deduction_overflows.add(sha256(bytearray.fromhex(f)).hexdigest())

#     if contract_address in list_of_skip_empty_string_literals:
#         with open(file) as f:
#             list_of_bytecode_skip_empty_string_literals.add(sha256(bytearray.fromhex(f)).hexdigest())

#     if contract_address in list_of_hidden_state_updates:
#         with open(file) as f:
#             list_of_bytecode_hidden_state_updates.add(sha256(bytearray.fromhex(f)).hexdigest())

#     if contract_address in list_of_straw_man_contracts:
#         with open(file) as f:
#             list_of_bytecode_straw_man_contracts.add(sha256(bytearray.fromhex(f)).hexdigest())
    
#     if contract_address in list_of_map_key_encodings:
#         with open(file) as f:
#             list_of_bytecode_map_key_encodings.add(sha256(bytearray.fromhex(f)).hexdigest())

#     if contract_address in list_of_racing_time_contracts:
#         with open(file) as f:
#             list_of_bytecode_racing_time_contracts.add(sha256(bytearray.fromhex(f)).hexdigest())

# if len(addresses) != len(list_of_bytecode_txhashes):
#     print("Error")
...
#     print("Error")
# if len(list_of_map_key_encodings) != len(list_of_bytecode_map_key_encodings):
#     print("Error")
# if len(list_of_racing_time_contracts) != len(list_of_bytecode_racing_time_contracts):
#     print("Error")
...
# overall_map_key_encodings          = 0
# overall_racing_time_contracts      = 0

# # cursor = collection.find()

# with open('ethereum_dataset.csv') as f: 
#     csvreader = csv.DictReader(f)
#     for row in csvreader:
# # for contract in cursor:
#         if row["Contract Hash"] in list_of_bytecode_txhashes:
#             overall_txhashes += 1
#         if row["Contract Hash"] in list_of_bytecode_money_flows:
#             overall_money_flows += 1
#         if row["Contract Hash"] in list_of_bytecode_balance_disorders:
#             overall_balance_disorders += 1
#         if row["Contract Hash"] in list_of_bytecode_hidden_transfers:
#             overall_hidden_transfers += 1
#         if row["Contract Hash"] in list_of_bytecode_inheritance_disorders:
#             overall_inheritance_disorders += 1
#         if row["Contract Hash"] in list_of_bytecode_uninitialised_structs:
#             overall_uninitialised_structs += 1
#         if row["Contract Hash"] in list_of_bytecode_type_deduction_overflows:
#             overall_type_deduction_overflows += 1
#         if row["Contract Hash"] in list_of_bytecode_skip_empty_string_literals:
#             overall_skip_empty_string_literals += 1
#         if row["Contract Hash"] in list_of_bytecode_hidden_state_updates:
#             overall_hidden_state_updates += 1
#         if row["Contract Hash"] in list_of_bytecode_straw_man_contracts:
#             overall_straw_man_contracts += 1
#         if row["Contract Hash"] in list_of_bytecode_map_key_encodings:
#             overall_map_key_encodings += 1
#         if row["Contract Hash"] in list_of_racing_time_contracts:
#             overall_racing_time_contracts += 1

# print("Number of overall vulnerable contracts: "+str(overall_txhashes))
...
# print("Number of overall map key encodings: "+str(overall_map_key_encodings))
# print("Number of overall racing time contracts: "+str(overall_racing_time_contracts))
...
#     file.write(address+"\n")
# file.close()
# file = open("map_key_encodings.txt", "w")
# for address in list_of_map_key_encodings:
#     file.write(address+"\n")
# file.close()
# file = open("racing_time_contracts.txt", "w")
# for address in list_of_racing_time_contracts:
#     file.write(address+"\n")
# file.close()