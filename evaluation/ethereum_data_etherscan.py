...
class EthereumData:
	...
	def find_address_from_txhash(self, tx_hash):
		# https://api.etherscan.io/api?module=proxy&action=eth_getTransactionReceipt&txhash=0xfb2be264a3607b6ebce41c034d42ad4358ce3b0514b01a7825db787ec865d689&apikey=YourApiKeyToken
		url = self.apiDomain + '?module=proxy&action=eth_getTransactionReceipt&txhash=' + tx_hash + '&apikey=' + self.apikey
		request_status = requests.get(url)
		if request_status.status_code != 200:
	...
	def contract_has_source_code(self, address):
        # https://api.etherscan.io/api?module=contract&action=getabi&address=0x525b988e7e2336C50bf9A05ad5F0cab6114C17BE&apikey=VT4IW6VK7VES1Q9NYFI74YKH8U7QW9XRHN
		url = self.apiDomain + '?module=contract&action=getabi&address=' + address + '&apikey=' + self.apikey
		request_status = requests.get(url)
		if request_status.status_code != 200:
			print("etherscan api error")
		else:
			contractinfo = request_status.json()
		info_result = contractinfo["result"]
		if info_result == "Contract source code not verified":
			return False
		return True